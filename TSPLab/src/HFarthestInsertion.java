...
public class HFarthestInsertion {
	...
	public void initializeWithZeroandFarthest(int nodeIni) throws Exception {
		...
		m_solution.setVertexPosition(nodeIni, 0); // it sets initial node at
													// position 0
		Visited.add(nodeIni);
		...
		m_solution.setVertexPosition(idxmax, 1); // it sets vertex idxmax at
													// position 1
		m_solution.setVertexPosition(nodeIni, 2); // it sets vertex idxmax at
													// position 2
		Visited.add(idxmax);
	...
	public void solve() throws Exception {
		...
		while (iterations > 0) { // all vertices are visited
			...
			// it finds the minimum way to visit the farthest vertex (bestposition)
			...
			for (int i = 1; i <= Visited.size(); i++) { // positions
				...
				long insertionValue = m_instance.getDistances(m_solution.getSolution(i - 1), maxVertex)

...
/**
 * The Instance class allows to create an object that contains the data stored in a tsp file.
 * <br> 
 *<br>  Only 2D EUCLIDEAN problems can be read, that is problems where the customer coordinates are given and the distance between
 * two customers is the euclidean distance.   
 * <br> 
 * <br> The class is created through its constructor that takes the data file as parameter. 
 * The data file is read and the data are stored in the Instance object. The data can then be access 
...
public class Instance {
  ...
  /** @return the number of vertices in the problem */
   ...
   /**
   * @param i vertex number (should be defined between 0 an the number of vertices in the problem minus one).
   *  Note the vertex number is not the label !
   * @return the x coordinate of vertex i.
   **/
	public double getX(int i) throws Exception {
		if ((i<0) || (i>=m_nbVertices))
      throw new Exception("Error : vertex index " + i + " should range between 0 and " + (m_nbVertices - 1)+".");
		return m_x[i];
	}
  ...
  /**
   * Returns the euclidean distance rounded to the nearest integer value between two vertices.
   * All distances are calculated when the tsp file is loaded, so this function does not calculate distances.
   Note: problems are symmetric, the distance from vertex i to vertex j is equal to the distance from j to i.
   * 
   * @param i origin vertex (should range between 0 and nbVertex-1).
   * @param j destination vertex (should range between 0 and nbVertex-1).
   * @return Returns the euclidean distance from i to j rounded to the nearest integer value
   * @throws Exception returns an error if i or j are not valid vertex numbers.
   **/
	public long getDistances(int i, int j) throws Exception {
		if ((i<0) || (i>=m_nbVertices))
		      throw new Exception("Error : vertex index " + i + " should range between 0 and " + (m_nbVertices - 1)+".");
		if ((j<0) || (j>=m_nbVertices))
		      throw new Exception("Error : vertex index " + j + " should range between 0 and " + (m_nbVertices - 1)+".");
  ...
  /**
   * Constructor: this method creates an object of class Instance. 
   * It calls the read method to load the data file given as parameter.
   * 
   * @param fileName 
   * 			instance file
   * @throws IOException
  ...
  // --------------------------------------
	...
	private void read() throws IOException {
		...
		if (!mfile.exists()) {
			throw new IOException("The file : "+ m_fileName + ", doesn«t exist");
		}

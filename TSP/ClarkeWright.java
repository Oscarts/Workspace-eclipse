...
public class ClarkeWright {
	...
	/*
	 * stores the different routes
	 * route1  0-2-3-4-0 is represented by {2,3} in the first row of m_routes
	 * */
	...
	public ClarkeWright(Instance instance) {
		...
		m_routes = new  ArrayList<ArrayList<Integer> > (m_instance.getNbVertices()-1);
	...
	/*
	 * it calculates the savings of all nodes, then put them in a list and sort them
	 */
	 ...
	 */
	public boolean mergeRoutes(ArrayList<Integer> route1, ArrayList<Integer> route2,int v1, int v2){
		// verifies that the two vertex aren«t in the same route
		if (m_routeNumberOfVertex[v1]==m_routeNumberOfVertex[v2]) {
			return false;
		}
		/*
		 * put at the end if it is at the beginning (reverse order), 
		 * nothing if it is in the middle and
		 *  do nothing if it is at the end
		 *   */
		if (route1.get(0)==v1){
		...
		for ( int i : route2){
			m_routeNumberOfVertex[i] = route1Index; //update route number to 1 for all vertex of route 2
		}
	...
	public void initializeRoutes(){
		...
		//System.err.println("Size of arrayList m_routes : " + m_routes.size());
...
//		}
		...
		while (m_nbRoutes>1) {
			...
			if (mergeRoutes(m_routes.get(m_routeNumberOfVertex[s.m_i]),
					   m_routes.get(m_routeNumberOfVertex[s.m_j]),
					   s.m_i,s.m_j)) {
				m_nbRoutes --;
			} 			
		}

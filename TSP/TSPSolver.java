...
//		
		...
		CW.solve();
		System.err.println("clarke & wright: " + CW.getSolution().getObjective());
		
//		Solution improvedSolution1=localSearch(CW.getSolution());
//		improvedSolution1.print(System.err);
//		m_solution=improvedSolution1;
//		
////		m_solution=CW.getSolution();
////		m_solution.evaluate();
...
//		System.err.println("finished!!!");
	...
	public Solution localSearch(Solution solution, int move) throws Exception{
		...
		System.err.println("local search sol:" + Sobj + " movement:" + move + " iterations:"+ iter);
		//S.print(System.err);
		return S;
	}
	...
	public  Solution orOpt(Solution solution) throws Exception{
		...
		for (int i = 1; i < m_instance.getNbVertices()-k+1; i++) { //different sequences
			for (int j = 1; j < m_instance.getNbVertices()-k+1; j++) { //position to insert sequences		
				...
				if (i!=j ){
					...
					if (j>i) {
						...
						//System.err.println("sequence " + i +" " + (i+1) +" "+ (i+2) +"position "+ j + "distance:" + tempObj+ " verif " );
